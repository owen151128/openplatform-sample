 ...
 * 자체인증 프래그먼트 추상 클래스
 */
public class AbstractSelfAuthMainFragment extends AbstractMainFragment {
    ...
    protected void showTokenDialog(EditText etToken) {
        showTokenDialog(etToken, null);
    }

    // 토큰선택창
    protected void showTokenDialog(EditText etToken, EditText etUserSeqNo) {
    ...
    /*
        SA 토큰발급 수행시 및 계좌등록시에만 해당. 필요한 데이터를 저장
        * SA 토클발급 응답형식
        {"access_token":"9c439c0c-32a2-4cd2-b0c3-74846b4828c8", "token_type":"Bearer",
         "expires_in":"7776000.0", "scope":"sa", "client_use_code":"201000"}
        * 계좌등록
    ...
    */
    ...
    protected void saveAuthData(String responseJson) {
        ...
        if (!accessToken.getAccess_token().isEmpty()) {
            SelfAuthUtils.saveSelfAuthToken(accessToken);
            Toast.makeText(context, "자체인증 access token 저장됨", Toast.LENGTH_SHORT).show();
        }

        // 다른 메뉴에서 user_seq_no 를 사용할 수 있도록 저장
        String userSeqNo = Utils.getValueFromJson(responseJson, "user_seq_no");
        if (!userSeqNo.isEmpty()) {
            Utils.saveData(SelfAuthConst.SELF_AUTH_USER_SEQ_NO, userSeqNo);
            Toast.makeText(context, "자체인증 사용자일련번호 저장됨", Toast.LENGTH_SHORT).show();
        }

        // (참고) 다른 메뉴에서 client_use_code 를 사용할 수 있도록 저장 -> 설정값에서 읽어오도록 변경(은행거래고유번호를 생성하려면 미리 알아야 함)
    }
    ...
    protected void showAccountDialog(EditText etFintechUseNum, EditText etBankCode, EditText etAccountNum) {
        ArrayAdapter<BankAccount> bankAccountAdapter = new ArrayAdapter<>(context, R.layout.simple_list_item_divider, R.id.text1, AppData.selfAuthBankAccountList);
        showAlertAccount(bankAccountAdapter, (parent, view, position, id) -> {
            ...
            if (bankAccount != null) {
                ...
                if (etAccountNum != null) {
                    etAccountNum.setText(bankAccount.getAccountNumFlex());
                }
            }
        });
    }
