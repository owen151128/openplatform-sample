 ...
 * 사용자정보조회, 등록계좌조회 어댑터
 */
public class SelfAuthAPIUserMeResultAdapter extends KmRecyclerViewArrayAdapter<BankAccount> {
    public SelfAuthAPIUserMeResultAdapter(ArrayList<BankAccount> itemList) {
        super(itemList);
    }
    ...
    public static class ApiCallUserMeResultViewHolder extends KmRecyclerViewHolder<BankAccount> {
        ...
        public ApiCallUserMeResultViewHolder(View v, SelfAuthAPIUserMeResultAdapter adapter) {
            super(v);
        ...
        public void onBindViewHolder(View view, int position, BankAccount item, boolean isSelected, boolean isExpanded, boolean isEtc) {
            ...
            // 계좌정보
            ((TextView) view.findViewById(R.id.tvAccountInfo)).setText(String.format("%s  %s", item.getAccountNumFlex(), item.getAccount_holder_name()));
            ...
            if (!item.getAccount_state().isEmpty()) {
                ...
                ((TextView) view.findViewById(R.id.tvAccountState)).setText(item.getAccountStateFlex());
            }
